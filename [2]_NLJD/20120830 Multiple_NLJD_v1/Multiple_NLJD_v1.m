...
load sample_dense.mat
plot(Measured(NLJD_Index,1),Measured(NLJD_Index,2),'r^');
hold on;
plot(Measured(Metal_Index,1),Measured(Metal_Index,2),'bo');
grid on;
axis([0 1000 0 1000]);
...
NLJD = [Measured(NLJD_Index,1) Measured(NLJD_Index,2)];
for idx_NLJD = 1:length(NLJD)
    Multiple_Training_NLJD(No_Training_Data*(idx_NLJD-1)+1:idx_NLJD*No_Training_Data,:) = repmat(NLJD(idx_NLJD,:),No_Training_Data,1) + (Training_Data_D .* randn(No_Training_Data,2));
end;
    
Metal = [Measured(Metal_Index,1) Measured(Metal_Index,2)];
for idx_Metal = 1:length(Metal)
    Multiple_Training_Metal(No_Training_Data*(idx_Metal-1)+1:idx_Metal*No_Training_Data,:) = repmat(Metal(idx_Metal,:),No_Training_Data,1) + (Training_Data_D .* randn(No_Training_Data,2));
end;

figure()
plot(Multiple_Training_NLJD(:,1),Multiple_Training_NLJD(:,2),'r^');
hold on;
plot(Multiple_Training_Metal(:,1),Multiple_Training_Metal(:,2),'bo');
grid on;
axis([0 1000 0 1000]);
...
for idx_k = 1:length(k)
    for idx_frame = 1:frame
        ...
        for idx_simulation = 1:knn_simulation_no
            ...
            fuzzy_Class = kNN(k(idx_k),Measured,fuzzy_cluster1,fuzzy_cluster2);
            
            %% kNN Classification and Decision
            kNN_Class = kNN(k(idx_k),Measured_Noise,Class1(:,1:2),Class2(:,1:2));
...
plot_data = [Prob_kNN_Class_Symmetric' Prob_kNN_Class_Fairness' Prob_kNN_Fuzzy'];

bar(k,plot_data);
legend('Conventional D/L','Proposed 1 (Fairness)','Proposed 2 (Fuzzy)');
axis([1 53 0 50]);
grid on;
