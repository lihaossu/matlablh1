...
for idx_R = 1:length(E_Ratio)
    ...
    Real_Data = [Data_nd Data_rd];
    
    Normalization = ceil(sqrt((Data_nd-Data_rd).^2+(Data_rd-Data_nd).^2));
    Noise1 = ((Noise_Variance)/100) * Normalization;
    Noise2 = Normalization-Noise1;
...
axis([0 E_Ratio(length(E_Ratio)) 0 100])
grid on
...
%%
figure()
plot([0:1:2000],[0:1:2000],'b:','LineWidth',2);
grid on;
