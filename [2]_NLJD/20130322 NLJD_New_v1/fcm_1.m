function [center, U, obj_fcn] = fcm_1(Database, cluster_n, options)
...
% 
% 
% for idx_fuzzy = 1:length(index1)
%     if Database(index1(idx_fuzzy),1:2) == Measured(idx_fuzzy,1:2)
%         decision_fuzzy(idx_fuzzy) = 0;
%     elseif Database(index2(idx_fuzzy),1:2) == Measured(idx_fuzzy,1:2)
%         decision_fuzzy(idx_fuzzy) = 1;
%     end;
% end;
% 
% 
% figure
% line(Database(index1,1),Database(index1,2),'linestyle','none','marker', 'x','color','r');
% line(Database(index2,1),Database(index2,2),'linestyle','none','marker', 'o','color','b');
% 
% hold on
% 
