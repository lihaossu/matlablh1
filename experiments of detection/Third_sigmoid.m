        ...
        label1 = zeros(1,length(Electronic_x));
        % A , B and C
        ...
        while mse_train > mse_thres && epoch <= num_Epoch
            ...
            for i = 1:num_tr,
                ...
                hd = [sigmoid(w1{1}*x);1];          % hidden neurons are nonlinea
                o  = sigmoid(w1{2}*hd);         % output neuron is nonlinear
                e(:,i)  = d - o;

                %% Backward computation
                delta_ou = e(:,i).*d_hyperlic(w1{2}*hd);            % delta for output layer
                delta_hd = d_sigmoid(w1{1}*x).*(w1{2}(:,1:n_hd)'*delta_ou);  % delta for hidden layer
        ...
        [x_b,y_b]= meshgrid(xmin:(xmax-xmin)/1000:xmax,ymin:(ymax-ymin)/1000:ymax);
        z_b  = 0*ones(size(x_b));
        %wh = waitbar(0,'Plotting testing result...');
        for x1 = 1 : size(x_b,1)
            for y1 = 1 : size(x_b,2)
                input = [(x_b(x1,y1)-mean1(1))/max1(1);(y_b(x1,y1)-mean1(2))/max1(2);1];
                hd= [sigmoid(w1{1}*input);1];
                z_b(x1,y1) = sigmoid(w1{2}*hd);%
            end
