...
%%%  carry out sas pca transform and pnn classification
%%%  input: data -- is an array of 12 vectors from leaf character parameters
%%%  output: classification_result -- the sequence number of leaf species
%   
%    created by Gang Wu
%    August 2006
%  
...
load five five;
save input input;
%%%%%%generalize is indispensable
five=five';
for m=1:5
input(m)=(input(m)-min(five(m,:)))/(max(five(m,:))-min(five(m,:)));%generalize
end
input=input';
y=sim(net2,input);
