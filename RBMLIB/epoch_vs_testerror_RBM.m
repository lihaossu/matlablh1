%%*******************************************************************************
% The number of epoch(iteration) VS error rate, namely 'Early stopping'
% probelm : we often observe that training error decreases steadily
% overtime, but validation begin to rise again after certain iteration. So
% we comsider the number of iteration VS the test error or training error?
% Result illustrates: No much effect b
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = (1 : num_tr),
    Metal1v_x1(i,:) = Metal_x1+ Standard_Noise*randn(1,1)+ Training_Standard_Deviation*randn(1,num_dim);
    Metal2v_x2(i,:) = Metal_x2 +Standard_Noise*randn(1,1)+ Training_Standard_Deviation*randn(1,num_dim);
    Metal1v_y1(i,:) = Metal_y1 +Standard_Noise*randn(1,1)+ Training_Standard_Deviation*randn(1,num_dim);
    Metal2v_y2(i,:) = Metal_y2 +Standard_Noise*randn(1,1)+ Training_Standard_Deviation*randn(1,num_dim);
    data1_metal(i,:) = Metal1v_x1(i,:) - Metal1v_y1(i,:);
    data2_metal(i,:) = Metal2v_x2(i,:) - Metal2v_y2(i,:);
...
maxepoch = 10:20:100;
for idx_maxepoch = 1:length(maxepoch)
X = data_tr_shuffled;
...
ccavg = cc;
t = 1;
errors = zeros(1,maxepoch(idx_maxepoch));


for epoch = 1 : maxepoch(idx_maxepoch)
...
%         cc = cc + ccinc;
    ...
    fprintf('Ended epoch %i/%i, Reconstruction error is %f\n', ...
            epoch, maxepoch(idx_maxepoch), errsum);
end
...
model.labels = u;
 ...
 % error rate
 yhat = prediction;
 Error(idx_maxepoch) = sum(yhat~=testlabels)/length(yhat);
 fprintf('Classification error using RBM with hiddens is %f\n', Error(idx_maxepoch));
 figure(1)
 hold on
 plot(errors,'k');
 title('Learning curve');
 xlabel('Number of iteration');ylabel('Reconstruction Error')
end
