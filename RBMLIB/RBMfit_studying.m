...
% additional inputs (specified a name value pairs or in struct)
% nclasses      ...number of classes
% method        ...CD or SML(???mostly use CD)
% eta           ...learning rate
% momentum      ...momentum for smoothness and to prevent overfitting 
%               ...NOTE:momentum is note recommend with SML
...
%model.wc       ...The weights of labels layer
%model.cc       ...The biases on labels layer

%errors         ...The erros in reconstruction at every epoch
...
for epoch = 1:maxepoch
    ...
    for batch = 1:numbatches
        ...
        % go up
        ph = logistic(data * W + classes*Wc + remat(b,numcases,1));
        ...
        %go down
        negdata = logistic(nhstates*W' + remat(c,numcases,1));
        negdatastates = negdata > rand(numcases,numdims);
        negclasses = softmaxPmtk(nhstates*Wc' + repmat(cc,numcases,1));
        negclasses = softmax_sample(negclasses);
        
        %go up one more time????????????????????????????????????????????
        nh = logistic(negdatastates*W + negclassesstacks*Wc + ...
            repmat(b,numcases,1));
        ...
        db = sum(ph) - sum(nh);
        dWc = sum(classes'*ph - negclassesstates'*nh);
        ...
        Wcinc = momentum*Wcinc + eta*(dW/numcases - penalty*Wc);
        ...
        %accumulate reconstraction error ???????????????????????????????
